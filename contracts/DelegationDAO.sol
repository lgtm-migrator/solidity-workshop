...
contract DelegationDAO is AccessControl {
    ...
    uint voteGrantAdminCount;
    uint voteRemoveAdminCount;
    uint memberTotal;
    ...
    constructor(address _target, address admin) {
        ...
        voteGrantAdminCount = 0;
        voteRemoveAdminCount = 0;
        memberTotal = 0;
    }
    ...
    function vote_grant_admiin(address newAdmin) external onlyRole(MEMBER) {
        voteGrantAdminCount += 1;
        if (voteGrantAdminCount >= memberTotal / 2) {
            grant_admin(newAdmin);
            voteGrantAdminCount = 0;
        }
    }

    function vote_remove_admin(address exAdmin) external onlyRole(MEMBER) {
      voteRemoveAdminCount += 1;
      if (voteRemoveAdminCount >= memberTotal / 2) {
          grant_admin(exAdmin);
          voteRemoveAdminCount = 0;
      }
    }
